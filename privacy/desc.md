...
Stops Windows from correcting the system clock through network time sources (the clock continues running from the hardware clock, but it may become inaccurate and eventually cause incorrect timestamps, certificate/sign-in errors, invalid authentication codes, or unreliable scheduled tasks).
